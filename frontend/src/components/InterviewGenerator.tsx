  ...
  return (
    <div className="bg-white dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-900 rounded-2xl p-6 text-left space-y-6 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <span className="text-[10px] font-bold text-[#71717A] dark:text-zinc-500 uppercase tracking-widest block">AI Interview Prep</span>
          <span className="text-xs text-[#71717A] dark:text-zinc-400 font-light mt-0.5 block">Generate targeted mock interviews tailored to your stack</span>
        </div>
        ...
        {questions && (
          <button
            ...
            className="p-1.5 rounded-lg border border-zinc-200 dark:border-zinc-900 bg-white dark:bg-zinc-950 hover:border-zinc-300 dark:hover:border-zinc-800 text-[#71717A] dark:text-zinc-400 hover:text-[#09090B] dark:hover:text-zinc-250 transition-all flex items-center gap-1.5 text-[10px] font-semibold uppercase shadow-sm"
          >
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            {(['Easy', 'Medium', 'Hard'] as const).map((d) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-[10px] font-semibold uppercase tracking-wider transition-all ${
                  difficulty === d 
                    ? 'bg-white text-[#09090B] dark:bg-zinc-805 dark:text-zinc-100 shadow-sm' 
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-xl text-xs font-semibold bg-[#09090B] text-white hover:bg-[#18181b] dark:bg-zinc-100 dark:text-zinc-955 dark:hover:bg-zinc-200 transition-all h-9 px-4 disabled:opacity-50 active:scale-[0.98]"
          >
            {isGenerating ? (
              <>
        ...
        {questions && (
          <div className="space-y-4 pt-2">
            {questions.map((q) => (
              <div key={q.id} className="p-4 rounded-xl border border-zinc-200 dark:border-zinc-900 bg-[#F4F4F5]/30 dark:bg-zinc-950/20 space-y-3">
                <div className="flex justify-between items-start gap-4">
                  <div className="flex flex-wrap gap-1.5">
                    <span className="px-2 py-0.5 rounded bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-850 text-[9px] font-bold uppercase tracking-widest text-[#71717A] dark:text-zinc-400 shadow-sm">
                      {q.category}
                    </span>
                    <span className={`px-2 py-0.5 rounded border text-[9px] font-bold uppercase tracking-widest ${
                      q.difficulty === 'Easy' ? 'bg-emerald-50 border-emerald-200 text-emerald-600 dark:bg-emerald-950/25 dark:border-emerald-900 dark:text-emerald-400' :
                      q.difficulty === 'Medium' ? 'bg-amber-50 border-amber-200 text-amber-600 dark:bg-amber-950/25 dark:border-amber-900 dark:text-amber-400' :
                      'bg-rose-50 border-rose-200 text-rose-600 dark:bg-rose-950/25 dark:border-rose-900 dark:text-rose-400'
                    }`}>
                      {q.difficulty}
                    </span>
                  </div>
                ...
                {visibleAnswers[q.id] && (
                  <div className="mt-2.5 p-3 rounded-lg border border-zinc-200 dark:border-zinc-900 bg-white dark:bg-zinc-950/40 text-[11px] text-[#71717A] dark:text-zinc-400 font-light leading-relaxed whitespace-pre-wrap shadow-sm">
