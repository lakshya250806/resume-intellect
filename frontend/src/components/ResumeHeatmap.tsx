...
export default function ResumeHeatmap() {
  ...
  const getStatusColor = (score: number) => {
    ...
    if (score >= 60) return { bg: 'bg-amber-50 border-amber-200 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-955', dot: 'bg-amber-500', label: 'Moderate', text: 'text-amber-600 dark:text-amber-500' };
    return { bg: 'bg-rose-50 border-rose-250 text-rose-600 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-955', dot: 'bg-rose-500', label: 'Weak', text: 'text-rose-600 dark:text-rose-500' };
  };
  ...
  return (
    <div className="bg-white dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-900 rounded-2xl p-6 text-left space-y-6 shadow-sm hover:shadow-md transition-shadow">
      <div>
        ...
        <span className="text-xs text-[#71717A] dark:text-zinc-400 font-light mt-0.5 block">Automated confidence weights per resume component</span>
      </div>
      ...
      <div className="space-y-4">
        {sections.map((sec, i) => {
          ...
          return (
            <div key={i} className="p-4 rounded-xl border border-zinc-200 dark:border-zinc-900 bg-[#F4F4F5]/30 dark:bg-zinc-955/20 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-1 max-w-xl">
                ...
                <div className="text-[10px] text-[#71717A] dark:text-zinc-450 font-normal flex items-start gap-1.5 pt-1.5 border-t border-zinc-200 dark:border-zinc-900/50 mt-1.5">
                  {sec.score >= 80 ? (
              ...
              <div className="flex items-baseline gap-1 self-end md:self-center">
                ...
                <span className="text-[10px] text-[#71717A] dark:text-zinc-500 font-light">/100</span>
              </div>
            </div>
          );
        })}
